...
def costfunc(prob,answer):
    ...
    return cost;
avg_expense=0;
# practise 60000 times
practise=0
    # new pixels
    #starter
while practise<60000 :
    ans=math.floor(random.random()*10);
    n=math.floor(random.random()*1000);
    random_file=(realset.files[ans][n]);
    # Get pixels list here
    ...
    while i<16:
        ...
        while j<784:
            k=0;
            while k<784:
                weights2[i][j]=weights2[i][j]+(pixels[(math.floor(j/28))][j%28]*neuron2grad[i])/1000;
                k=k+1;
            j=j+1;
        i=i+1;
    i=0;
    while i<16:
        ...
        while j<16:
            k=0;
            while k<16:
                weights3[i][j]=weights3[i][j]+(neuron2[k]*neuron3grad[i])/1000;
                k=k+1;
            j=j+1;
        i=i+1;
    i=0
    while i<10:
        ...
        while j<16:
            k=0;
            while k<16:
                weights4[i][j]=weights4[i][j]+(neuron3[j]*neuron4grad[i])/1000;
                k=k+1;
            j=j+1;
        i=i+1;
    practise=practise+1;
    expense=costfunc(neuron4,answer)
    avg_expense=avg_expense+expense;
    k,max=maxi(neuron4);
    
    if (practise % 200==0):

        print('');
        print(practise,' - ',ans,' - ',expense,' my guess is - ',k,' - ',avg_expense/200,end='');
        avg_expense=0
